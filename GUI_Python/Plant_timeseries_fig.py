...
def Plant_timeseries_fig(id_str: str, file_path_root: Path,):
    g01_fname = file_path_root / f"{id_str}.G01"
    ...
    ax[2].set_title('Water Stress Index [actual Tr. / potential Tr.]')
    ...
    plt.tight_layout()
    fig_fname = Path(file_path_root) / f"{id_str}.LAI_WStress_timeseries.jpg"
    ...
    plt.tight_layout()
    fig_fname = Path(file_path_root) / f"{id_str}.DryMatter_timeseries.jpg"
    plt.savefig(fig_fname) #, dpi=150)
    plt.close(fig)
    # plt.show()