...
#==============================================================================
#Climate file
def SM_timeseries_fig(id_str: str, file_path_root: Path,):
    g03_fname = file_path_root / f"{id_str}.G03"
    g07_fname = file_path_root / f"{id_str}.G07"
    grd_fname = file_path_root / f"{id_str}.grd"
    ini_fname = file_path_root / f"{id_str}.ini"

    fig, ax = plt.subplots(3,1,figsize=(9,8))  #two years of data together => total 11 years
    fig.suptitle('Soil water content:  ' +id_str) #g03_fname[-12:-4])
    ...
    df_g07 = pd.read_csv(g07_fname, parse_dates=['           Date']) #, delim_whitespace=False) #, sep=r'\s+')
    ...
    ax[0].plot(xdata, ydata1,'-', color=c_list[0], marker=marker_list[0],markersize=3,label = 'Total profile water') #1) total profile NO3
    #======================================================
    # Group by layer
    ...
    plt.tight_layout()
    fig_fname = Path(file_path_root) / f"{id_str}.sm_by_depth_timeseries.jpg"
    ...
    fig.suptitle('Soil NO3 content:  ' +id_str) #g03_fname[-12:-4])
