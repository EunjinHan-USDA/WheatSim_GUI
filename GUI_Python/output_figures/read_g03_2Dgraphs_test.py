...
working_dir = 'D:\\ACSL_EJ\\Irrigation_DST\\Excel2Python\\SB_sim_test\\2002_103_NTSB\\'
...
fname = os.path.join(working_dir, "2002_103_NTSB.G03") #<<<=====================
...
date1 = pd.Timestamp(2002,5,14) #<<<==5/14 start time, 5/21 sowing
date2 = pd.Timestamp(2002,10,3) #<<<=====================
# date1 = pd.Timestamp(2002,9,27) #<<<=====================
# date1 = pd.Timestamp(2006,5,7) #<<<=====================
# date2 = pd.Timestamp(2002,9,27) #<<<=====================
...
for ii, (target_date, df3) in enumerate(df2.groupby('Date', sort=True)):
    ...
    for var, i in zip(param, range(0,4)):
        ...
        if var == "hNew":
            ...
            vmin = 10
            vmax = 30
            levels = np.linspace(10.0,30.0, 19)   #<<<=====================
